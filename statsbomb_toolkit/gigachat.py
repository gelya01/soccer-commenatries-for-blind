...
def make_prompt(event_json: dict[str, Any], sb360_json: dict[str, Any] | None) -> str:
    return f"""Ты футбольный комментатор.
Определи, какое действие сейчас происходит, и прокомментируй его как в лайв-репортаже.
Я должна понимать игру так, будто бы я матч визуально посмотреть не могу.
Не выдумывай действия и факты в комментариях, которые ты не можешь подтвердить исходя из данных.

CAMERA / ORIENTATION:
Все координаты УЖЕ приведены к единой ориентации (standardized / flip applied).
После стандартизации:
- x=0 - ворота Шотландии
- x=120 - ворота Германии
- y=0 верхняя бровка на карте, y=80 нижняя бровка
Называй игроков по фамилиям. Не нужно каждый раз называть команду.

ПРАВИЛА ОПИСАНИЯ:

- Нужно кратко, но точно описывать происходящее событие в одно предложение.
КАК ОПИСЫВАТЬ ДЕЙСТВИЕ:
- Используй глаголы: "делает передачу", "ведёт мяч", "бьёт", "принимает", "теряет", "перехватывает"

Верни СТРОГО валидный JSON (без markdown, без лишнего текста):
...
timestamp и action бери из event_json
"""
...
class GigaChatClient:
    def __init__(
        ...
        *,
        ...
        if verify_ssl_certs:
            self.verify: bool | str = str(ca_bundle) if ca_bundle and Path(ca_bundle).exists() else True
        else:
            self.verify = False
    ...
    def _oauth_headers(self, scheme_override: str | None = None) -> dict[str, str]:
        headers = {
            ...
            "Authorization": self._oauth_authorization_value(scheme_override=scheme_override),
        }
        if self.x_client_id:
    ...
    def _raise_api_error(self, resp: requests.Response, context: str) -> None:
        ...
        if isinstance(payload, dict):
            message = payload.get("message") or payload.get("error_description") or payload.get("error") or str(payload)
        else:
            message = str(payload)
        raise GigaChatAPIError(f"{context} failed: HTTP {resp.status_code}: {message}")

    def _authorized_headers(self, access_token: str, content_type: str | None = "application/json") -> dict[str, str]:
    ...
    def chat_completion(
        ...
        *,
        ...
        if messages is None:
            ...
            if not user_msgs:
                raise ValueError("attachments were provided but no user message in messages.")
    ...
    def generate_commentary(
        ...
        *,
        ...
        action = self._event_action_from_event_json(event_json) or str(parsed.get("action", ""))
